...
async def validate_quote_access(conn, quote_id: UUID, user: User, action: str = "view"):
    ...
    query = """
        SELECT q.id, q.organization_id, q.customer_id, q.created_by,
               q.idn_quote, q.title, q.description, q.status,
               q.workflow_state,
...
async def list_quotes(
    ...
    try:
        ...
        query = supabase.table("quotes").select(
            "id, idn_quote, customer_id, created_by, title, description, status, workflow_state, "
        ...
        if search:
            # Search in idn_quote, title, AND customer name
...
async def create_quote(
    ...
    Generates IDN (Identification Number) if customer has INN and org has supplier_code.
    """
    ...
    try:
        ...
        if quote_data.customer_id:
            customer_row = await conn.fetchrow(
                "SELECT id, inn FROM customers WHERE id = $1",
            ...
            customer_inn = customer_row['inn']

        # Generate IDN if customer has INN
        idn_quote = None
        if customer_inn:
            try:
            ...
            except IDNValidationError as e:
                # Log but don't fail - INN format issue
                import logging
                logging.warning(f"IDN generation skipped: {e}")
            except IDNGenerationError as e:
                # Log but don't fail - org may not have supplier_code yet
                import logging
                logging.warning(f"IDN generation skipped: {e}")

        # Insert new quote with IDN
        query = """
            INSERT INTO quotes (
                idn_quote, user_id, customer_id, customer_name, customer_email, customer_address,
        ...
        row = await conn.fetchrow(
            query,
            idn_quote,  # New: IDN quote identifier
...
async def delete_quote(
    ...
    try:
        ...
        return SuccessResponse(
            message=f"Quote {quote_data['idn_quote']} deleted successfully"
        )
...
async def soft_delete_quote(
    ...
    try:
        ...
        result = supabase.table("quotes").select("id, idn_quote, organization_id, deleted_at").eq("id", str(quote_id)).execute()
...
async def list_bin_quotes(
    ...
    try:
        ...
        # Explicitly list fields to ensure new columns are included
        query = supabase.table("quotes").select(
            "id, idn_quote, customer_id, title, description, status, "
            "quote_date, valid_until, currency, "
...
async def add_quote_item(
    ...
    try:
        ...
        # Validate quote access and editability - get quote data for IDN
        quote_data = await validate_quote_access(conn, quote_id, user, "edit")
        ...
        # Determine position - use provided or get next
        ...
        # Generate IDN-SKU if quote has IDN
        idn_sku = None
        if quote_data.get('idn_quote'):
            idn_service = get_idn_service()
            idn_sku = idn_service.generate_idn_sku(quote_data['idn_quote'], position)

        # Insert new item (calculations will be handled by database trigger)
        query = """
            INSERT INTO quote_items (
                quote_id, idn_sku, description, product_code, category, brand, model,
...
async def submit_quote_for_financial_approval(
    ...
    try:
        ...
        return SuccessResponse(
            message=f"Quote {quote['idn_quote']} submitted for financial approval"
        )
...
async def approve_quote_financially(
    ...
    try:
        ...
        quote_result = supabase.table("quotes").select("workflow_state, idn_quote")\
            .eq("id", str(quote_id))\
        ...
        return SuccessResponse(
            success=True,
            message=f"КП {quote['idn_quote']} финансово утверждено"
        )
...
async def reject_quote_financially(
    ...
    try:
        ...
        return SuccessResponse(
            success=True,
            message=f"КП {quote['idn_quote']} отклонено финансовым менеджером"
        )
...
async def send_quote_back_for_revision(
    ...
    try:
        ...
        return SuccessResponse(
            success=True,
            message=f"КП {quote['idn_quote']} отправлено на доработку"
        )
...
async def submit_quote_for_approval(
    ...
    try:
        ...
        return SuccessResponse(
            message=f"Quote {quote_data['idn_quote']} submitted for approval to {len(approval_request.approver_ids)} managers"
        )
...
async def approve_quote(
    ...
    try:
        ...
        updated_quote = await conn.fetchrow(
            "SELECT idn_quote, status FROM quotes WHERE id = $1",
            quote_id
        )
        ...
        return SuccessResponse(
            message=f"Quote {updated_quote['idn_quote']} {approval_update.approval_status}. New status: {updated_quote['status']}"
        )
...
async def send_quote_to_customer(
    ...
    try:
        ...
        return SuccessResponse(
            message=f"Quote {quote_data['idn_quote']} sent to customer {quote_data['customer_name']}"
        )
...
async def mark_quote_accepted(
    ...
    try:
        ...
        return SuccessResponse(
            message=f"Quote {quote_data['idn_quote']} marked as accepted by customer"
        )
...
async def generate_quote_pdf(
    ...
    try:
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=quote_{quote_row['idn_quote']}.pdf"
            }
        )
...
async def export_quote_pdf(
    ...
    try:
        ...
        idn_quote = export_data.quote.get('idn_quote', 'quote')
        # Clean quote number (remove 'КП-' prefix if present)
        idn_quote_clean = str(idn_quote).replace('КП-', '').replace('КП', '')
        filename = f"kvota_{format_name}_{quote_date}_{idn_quote_clean}_{customer_name_clean}.pdf"
...
async def import_items_to_quote(
    ...
    try:
        ...
        # Get IDN service if quote has IDN
        idn_service = get_idn_service() if quote_data.get('idn_quote') else None
        ...
        for idx, item in enumerate(items, start=1):
            try:
                ...
                # Generate IDN-SKU if quote has IDN
                idn_sku = None
                if idn_service and quote_data.get('idn_quote'):
                    idn_sku = idn_service.generate_idn_sku(quote_data['idn_quote'], position)
        ...
        return {
            "success": True,
            "message": f"Successfully imported {len(added_items)} items to quote {quote_data['idn_quote']}",
...
async def export_quote_excel(
    ...
    try:
        ...
        filename = f"kvota_{format_suffix}_{quote_date}_{idn_quote_clean}_{clean_customer}.xlsx"
